 ...
 */
    ...
    public:
        ...
        typedef set<DPIm> DPTItemSet;
        typedef OrderedMap<DPIm,DPTItemSet> DPTItemToDPTItemSetMap;
        ...
        typedef OrderedMap<const SVFGNode*,set<CallStrCxt>> DPMToContextStringSetMap;
        ///Constructor
        ...
        virtual const CPtSet& findPT(const DPIm& dpm)
        {
            if(isbkVisited(dpm))
            {
                ...
                //auto tempFinals = getFinalDPImSet(dpm);
                //finalSet.insert(tempFinals.begin(), tempFinals.end());
                return cpts;
            }
    ...
    protected:
        ...
        virtual void handleSingleStatement(const DPIm& dpm, CPtSet& pts)
        {
            ...
            else if(SVFUtil::isa<StoreSVFGNode>(node))
            {
                if(isMustAlias(getLoadDpm(dpm),dpm))
                {
                    ...
                    const PAGNode* pagNode = _svfg->getLHSTopLevPtr(getLoadDpm(dpm).getLoc());
        ...
        virtual inline void resetQuery(bool clearAll)
        {
            if(outOfBudgetQuery)
                OOBResetVisited();
...
//                return;
        ...
        inline void backTraceForInsertVal(CPtSet& pts, const DPIm& oldDpm){
            const InsertValVFGNode* ivNode = SVFUtil::cast<InsertValVFGNode>(oldDpm.getLoc());
            //assert(isTrackingExtractVal && "tracking insertVal without extractVal");
            if(extractInsertStack.empty()){
                backtraceAlongDirectVF(pts,oldDpm);
            }else if(extractInsertStack.top().ls == ivNode->getLocationSet() && extractInsertStack.top().extractSrcType == ivNode->getRes()->getType()){
                const SVFGNode* src = getSVFG()->getDefSVFGNode(ivNode->getInserted());
                const SVFGEdge* edge = getSVFG()->getIntraVFGEdge(src, ivNode,SVFGEdge::IntraDirectVF);
                assert(edge && "edge not found");
                extractInsertStack.pop();
                isTrackingExtractVal = false;
                backwardPropDpm(pts,ivNode->getInserted()->getId(),oldDpm,edge);
            }else {/*if(ivNode->getParent()){
                const SVFGNode* src = getSVFG()->getDefSVFGNode(ivNode->getParent());
                const SVFGEdge* edge = getSVFG()->getIntraVFGEdge(src, ivNode,SVFGEdge::IntraDirectVF);
                assert(edge && "edge not found");
                backwardPropDpm(pts,ivNode->getParent()->getId(),oldDpm,edge);
            }else{*/
                backtraceAlongDirectVF(pts,oldDpm);///TODO: ensure this works
            }
        }

        inline void backTraceForExtractVal(CPtSet& pts, const DPIm& oldDpm){
            const ExtractValVFGNode* evNode = SVFUtil::cast<ExtractValVFGNode>(oldDpm.getLoc());
            ExtractInsertRec eiRec;
            eiRec.ls = SVFUtil::cast<ExtractValPE>(evNode->getPAGEdge())->getLocationSet();
            eiRec.extractSrcType = evNode->getPAGSrcNode()->getType();
            extractInsertStack.push(eiRec);
            isTrackingExtractVal = true;
            extractInsertLs = SVFUtil::cast<ExtractValPE>(evNode->getPAGEdge())->getLocationSet();
            const SVFGNode* src = getSVFG()->getDefSVFGNode(evNode->getPAGSrcNode());
            const SVFGEdge* edge = getSVFG()->getIntraVFGEdge(src, evNode,SVFGEdge::IntraDirectVF);
            assert(edge && "edge not found");
            backwardPropDpm(pts,evNode->getPAGSrcNode()->getId(),oldDpm,edge);
        }
        ...
        virtual void backwardPropDpm(CPtSet& pts, NodeID ptr,const DPIm& oldDpm,const SVFGEdge* edge)
        {
            ...
            //copyFinalDpmSet(oldDpm,dpm); ///MPKRustIsolation
        }
        ...
        virtual void addCallString(const DPIm&){
            return;
        }
        /// Update call graph
    ...
    protected:
        ...
        void addFinalStackDpm(const DPIm& dpm){
            FinalStackDpmSet.insert(dpm);
        }

        void addFinalHeapDpm(const DPIm& dpm){
            FinalHeapDpmSet.insert(dpm);
        }

        const DPTItemSet& getFinalStackDpmSet(){
            return FinalStackDpmSet;
        }

        const DPTItemSet& getFinalHeapDpmSet(){
            return FinalHeapDpmSet;
        }
        ...
        LocationSet extractInsertLs;
        bool isTrackingExtractVal = false;
        DPMToContextStringSetMap contextStringMap;
        struct ExtractInsertRec{
            LocationSet ls;
            const Type* extractSrcType;
        };
        stack<ExtractInsertRec> extractInsertStack;
        DPTItemToDPTItemSetMap dpmToFinalDpmSet;
        DPTItemSet FinalHeapDpmSet;
        DPTItemSet FinalStackDpmSet;
    };
