...
cargo clean
env RUST_BACKTRACE=1 RUSTFLAGS="-C codegen-units=1 -C save-temps -Zprint-link-args --emit=llvm-bc -C trust -Clink-args=-Wl,-rpath=$BENCH_HOME/../../mpk-mimalloc/out/release -L/$BENCH_HOME/../../mpk-mimalloc/out/release -lmimalloc -Clink-args=-Wl,-rpath=$BENCH_HOME/../../mpk-library/rust-lib/target/release -L/$BENCH_HOME/../../mpk-library/rust-lib/target/release -lrustfuncs" cargo bench --features full --no-run  --target x86_64-unknown-linux-gnu
rm $BENCH_HOME/target/x86_64-unknown-linux-gnu/release/deps/*no-opt*
$BENCH_HOME/../../mpk-svf/Release-build/bin/dvf -cxt -max-cxt=2 -flow-bg=10 -field-limit=0 -cxt-bg=10 -query metasafe $(find $BENCH_HOME/target/x86_64-unknown-linux-gnu/release/deps -name '*.bc')
